...
function changeFunc(message) {
    ...
    if (message.includes("lead.")) {
        ...
        ZOHO.CRM.API.getRecord({ Entity: "Deals", RecordID: Const.idModulo })
            .then(function(data) {
                ZOHO.CRM.API.getRecord({ Entity: "Contacts", RecordID: data.data[0].Contact_Name.id })
                    .then(function(contact) {
                        ZOHO.CRM.API.getRecord({ Entity: "Accounts", RecordID: data.data[0].Account_Name.id })
