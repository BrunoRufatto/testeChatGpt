class customAction {
    ...
    async verifyNewVersion(){
        ZOHO.CRM.API.getOrgVariable(Const.apinames.apidparameters).then(function(data){
            ...
            if(config.hasOwnProperty("devices")){
                ...
                var device_saved = Const.actualFields.device_id;
                if(devices.length > 0){
                    Const.newversion = true;
                    var fldSel = '<select id="deviceSelected" class="newSelect" style="width:300px" required="true" >';
                    var lContinua = false;
                    devices.forEach(device => {
                        if(device.status){
                            lContinua = true;
                            if(device_saved == device.device_id){
                                fldSel = fldSel + '<option value="'+device.device_id+'" selected>'+device.device_name + ' (' + device.phone +')</option>';
                            } else{
                                fldSel = fldSel + '<option value="'+device.device_id+'">'+device.device_name + ' (' + device.phone +')</option>';
                            }
                        }
                    });
                    if(lContinua){
                    fldSel = fldSel + '</select>';
                    $("#divDevice").html(fldSel);
                    $("#deviceSelected").multipleSelect({
                        filter: true,
                    });
                    $("#showDeviceList").show();
                    } else{
                        Utils.errorMsg("ErroMsgGrande", "msg-error-device-app");
    ...
    async activeFields() {
        ...
        if (optionSelected.toLowerCase() == "leads") {
            $("#modifiedNote").on("keydown", function(e) {
                ...
                if ((e.which == 51 || e.which == 222) && e.key == "#") { //} && e.val().includes("#")) { //shift+#
    ...
    async loadingAllFields() {
        ...
        if (optionSelected.toLowerCase() == "contacts") {
        ...
        if (optionSelected.toLowerCase() == "accounts") {
        ...
        if (optionSelected.toLowerCase() == "deals") {
            ZOHO.CRM.META.getFields({
                "Entity": "Deals"
            }).then(function(data) {
                var resp = data;
    ...
    async validateModule() {
        ...
        if (this.config.module.toLowerCase() == "leads" || this.config.module.toLowerCase() == "contacts" || this.config.module.toLowerCase() == "accounts" || this.config.module.toLowerCase() == "deals") {
            ...
            var moduleapi = "api" + this.config.module.toLowerCase();
            await ZOHO.CRM.API.getOrgVariable(Const.apinames[moduleapi]).then(async function(data) {
                var resp = JSON.parse(data.Success.Content).name;
                if (resp.toLowerCase() == "-none-" || resp.toLowerCase().includes("none") || resp == null || resp == "" || resp == undefined) {
                    $('#newLoad').delay(1000).slideUp()
                    var msg = Const.desclanguage["custom-action-error-msg-fields"];
    ...
    async loadParams() {
        ...
        return ZOHO.CRM.API.getOrgVariable(Const.apinames.apidparameters).then(function(data) {
            try {
                ...
                Const.optionParam = JSON.parse(Const.optionParam);
                if (typeof Const.optionParam === "string"){
                    Const.optionParam = JSON.parse(Const.optionParam);
                }
                Const.jsonParam = Const.optionParam.allow;
    ...
    async loadFields(cont) {
        ...
        var radioChecked = $("input[name=gender]:checked")
    ...
    async saveAction() {
        ...
        switch (radioVal) {
            case 'template':
                switchcheck = $("#myonoffswitchtemplate").is(':checked');
                quantity = $("#quantityTemplate").val();
                ...
                if (temptext == "" || temptext == null || temptext == "undefined" || temptext == "none") {
                    ...
                    if(Const.newversion){
                        var device_id = $("#deviceSelected option:selected").val();
                        Const.actualFields.device_id = DOMPurify.sanitize(device_id,Const.configPurify);
                    }
                    Const.actualFields.bot = switchcheck;
                    Const.actualFields.nivel = quantity;
                }
                break;
            case 'custom':
                switchcheck = $("#myonoffswitchmanual").is(':checked');
                quantity = $("#quantityManual").val();
                ...
                if (mensagem == "" || mensagem == null || mensagem == "undefined") {
                    ...
                    Const.actualFields.module = DOMPurify.sanitize(Const.actualFields.module, Const.configPurify);
                    Const.actualFields.note = DOMPurify.sanitize(mensagem.replace(/\n/ig, ' \\n'), Const.configPurify);
                    if(Const.newversion){
                        var device_id = $("#deviceSelected option:selected").val();
                        Const.actualFields.device_id = DOMPurify.sanitize(device_id,Const.configPurify);
                    }
...
Const = {
    ...
    apinames: {
        ...
        apiaccounts: "whatsapphubforzohocrm__accounts",
        apicontacts: "whatsapphubforzohocrm__contacts",
        apideals: "whatsapphubforzohocrm__deals",
        apileads: "whatsapphubforzohocrm__leads"
    },
...
function botManual() {
    if ($("#myonoffswitchmanual").is(":checked")) {
        $("#nivelManual").show();
    } else {
        $("#nivelManual").hide();
    }
...
Utils.errorMsg = function (id, message, reload = false, close = false, refreshback = null) {
    ...
    $('#' + id).slideDown(function () {
        $('#' + id).delay(5000).slideUp(function () {
            $("#newLoad").hide();
            if (reload) {
                reloadWebtab();
            }
